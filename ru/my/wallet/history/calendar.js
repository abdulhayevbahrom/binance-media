...
function renderCalendar(type) {
    ...
    for (let day = 1; day <= lastDate; day++) {
        ...
        if (selectedDates[type] && selectedDates[type].year === year && selectedDates[type].month === month && selectedDates[type].day === day) {
...
function selectDate(type, year, month, day) {
    ...
    if (type === 'start') {
        document.getElementById("startDate").value = selectedDate.toISOString().split('T')[0];
        closeModal('start');
    } else if (type === 'end') {
        document.getElementById("endDate").value = selectedDate.toISOString().split('T')[0];
        closeModal('end');
    }
}
...
document.getElementById("startDate").addEventListener("click", () => openModal('start'));
document.getElementById("endDate").addEventListener("click", () => openModal('end'));
