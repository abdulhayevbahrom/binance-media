...
// downloadTransaction funksiyasini global qilish uchun
...
async function getTableData(selectedValues = {}, isPaginationChange = false) {
  ...
  let tx_type;
  const distribution = document.getElementById("distribution_history_airdrop_link");
  if (selectedValues.selectV4 === "Withdraw") {
    tx_type = "out";
    distribution.innerText = "Withdrawal hasn't arrived?"
  } else if (selectedValues.selectV4 === "all") {
    tx_type = "all";
  } else {
    tx_type = "in";
  }
  ...
  if (data.length === 0) {
    tableNoData.innerHTML = `
        <div class="flex_flex-col">
          ...
          <div class="body3 mt-4xs pc:mt-2xs text-t-third">No Data.</div>
        </div>
    `;
  } else {
...
function collectFormData() {
  ...
  if (formData.timeRange === "Customize") {
