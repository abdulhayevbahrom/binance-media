...
async function getTableData(selectedValues = {}) {
  ...
  switch (selectedValues.orderType2) {
    ...
    case "Последние 90 дней":
      ...
      break;
    case "Настроить":
      // "Настроить" tanlanganda selectedValues.dateRange ishlatiladi
      start_date = selectedValues.dateRange.split(" - ")[0];
      end_date = selectedValues.dateRange.split(" - ")[1];
      break;
    default:
      // Default holat: "Последние 30 дней"
      end_date = formatDate(today);
      start_date = formatDate(new Date(today.setDate(today.getDate() - 30)));
  }

  // tx_type ni orderType1 asosida aniqlash
  let tx_type = "in";
  if (selectedValues.orderType1 === "Вывод") {
    tx_type = "out";
  } else if (selectedValues.orderType1 === "Ввод") {
    tx_type = "in";
  }
  ...
  try {
    let response = await fetch(API, {
      ...
      body: JSON.stringify({
        ...
        end_date: end_date,
        tx_type: tx_type || "Вывод",
        language: "ru",
      }),
    });
    let data = await response.json();
    let slicedData = data.slice(0, 10);
    loadTableData(slicedData);
    loadTransactions();
  } catch (err) {
    console.log(">>", err);
  }
  ...
  if (data.length === 0) {
    ...
    data.forEach((transaction) => {
      ...
      row.innerHTML = `
        ...
        <td style="color: var(--color-Success)" data-label="Статус">${transaction.status}</td>
      `;
